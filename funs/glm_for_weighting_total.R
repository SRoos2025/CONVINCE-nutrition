#fit model for each imputation, and within imputation, apply on each visit
glm_for_weighting_total <- function(i) {
    dat_imp <- filter(data_unweighted, .imp == i)
    
    map(visits, function(v) {
        dat_imp_visit <- filter(dat_imp, visit == v)
        
        glm(
            inf_cens == 0 ~ age + sex + dial_vintage + group +
                uf_vol + sbp_pre + urea_pre_mgdl +
                phos_pre_mmoll + residual_urine_out + hdf_convol +
                dbp_pre + ktv_pre + hb_pre_mmoll + crp_pre_mgdl + k_pre_mmoll +
                SCR_MH_AP_OCCUR:SCR_MH_COPD_OCCUR,
            data = dat_imp_visit,
            family = binomial()
        )
    })
}
