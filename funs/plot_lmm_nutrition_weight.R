plot_lmm_nutrition_weight <- function(data, y_lab, y_limits) {
    ggplot(data, aes(x = visit_time, y = pred, color = factor(group), group = factor(group))) +
        ...
        geom_ribbon(aes(ymin = ll, ymax = ul, fill = factor(group)), #achter andere aes zetten
                    alpha = 0.2, color = NA) +
        labs(
            x = "Time (months)",
            y = y_lab,
        ...
        scale_x_continuous(breaks = seq(min(data$visit_time), max(data$visit_time), by = 3)) +
