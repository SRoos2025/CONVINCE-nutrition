#make a density plot comparing the propensity score of censored with uncensored without weighting
unweight_plot_ipcw <- function(data) {
    ggplot(data, aes(x = ps_ipcw, fill = as.factor(inf_cens))) +
        ...
        labs( x = "Propensity score", y = "Density", title = "Unweighted Distribution of propensity scores per imputation", fill = "Censoring status") +
        theme_minimal()+
        theme(
